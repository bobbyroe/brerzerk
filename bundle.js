...
function drawWalls (options_obj) {
		...
		if (tile.was_visited === true) {
			...
			if (tile.id !== start_id && tile.id === exit_id) {
				...
				tile.adjacent_tiles.forEach( t => {
					...
					let side = _getNeighborSideIndexFor(t, tile);
				...
				if (next_tile.was_visited === true) {
					// break down the wall if need be ... in the /next/ tile
					// "transpose" the side 
	...
	game_tiles.forEach( t => { 
		if (t.was_visited === false) { t.interior_walls.forEach( n => t.walls.push(n)); } 
	});
	...
	game_tiles.forEach( t => {
		...
		if (t.was_visited === false) {
			...
			let side = _getNeighborSideIndexFor(random_id, t);
